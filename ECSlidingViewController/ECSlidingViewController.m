...
/*
 Had to edit changes to this file in order to make it work for multiple versions for both iPhone and iPad.
 Where I have edited I commented as "Eddie Note:"
 */
...
- (CGRect)fullViewBounds
{
    ...
    //Eddie Note: Commented out this Landscape Orientation code since it was causing problems when switching orientations in iPad.
    /*
    if (UIInterfaceOrientationIsLandscape(sharedApplication.statusBarOrientation)) {
    ...
    */
    ...
    //Eddie Note: Commented out since it was giving problems with status bar.
    //bounds.size.height -= statusBarHeight;

    return bounds;
}

//Eddie Note: fullViewBoundsForMenu added.
- (CGRect)fullViewBoundsForMenu
{
    CGFloat statusBarHeight = 0.0f;
...
//Eddie Note: Modified updateUnderLeftLayout (but not updateUnderRightLayout since not using it) to use new fullViewBoundForMenu. method.
- (void)updateUnderLeftLayout
{
    if (self.underLeftWidthLayout == ECFullWidth) {
        [self.underLeftView setAutoresizingMask:self.autoResizeToFillScreen];
        [self.underLeftView setFrame:[self fullViewBoundsForMenu]]; //Eddie Note: Changed (Calling new method fullViewBoundForMenu)

    } else if (self.underLeftWidthLayout == ECVariableRevealWidth && !self.topViewIsOffScreen) {
        CGRect frame = [self fullViewBoundsForMenu]; //Eddie Note: Changed (Calling new method fullViewBoundForMenu)
